...
public class WorldMap
{
	...
	public GameShim game;
	public NetShim net;
	public ServerShim se;
	
	public Physics physics;
	Random randc = new Random();
	// CONSTRUCTORS

	public WorldMap(boolean _isServer, int _cachesize)
	{
		this(_isServer,_cachesize,false);
	}

	public WorldMap(boolean _isServer, int _cachesize, boolean multiMode)
	{
		this(_isServer,_cachesize,multiMode,"map");
	}

	public WorldMap(boolean _isServer, int _cachesize, String name)
	{
		this(_isServer,_cachesize,false,name);
	}

	public WorldMap(boolean _isServer, int _cachesize, boolean multiMode, String name)
	{
		mapName=name;
		isServer = _isServer;
		System.out.println("[MAP] Initializing '" + name + "'...");
		physics = new Physics(isServer);
		multiplayer = multiMode;
		chunks = new Chunk[_cachesize];
		cachesize = _cachesize;
		saveDir = "";
		for(int i=0;i<cachesize;i++)
		{
			chunks[i] = new Chunk(0,0,false);
	...
	// CHUNK LOCATING	
	public Chunk grabChunk(int x, int y) throws NoChunkMemException
	{
		// Is the chunk cached?
		...
		for (int i=0;i<cachesize;i++) if(!chunks[i].isSet) { chunks[i] = new Chunk(x,y,true); chunks[i].isUsed = true; readChunkFile(x,y,chunks[i]); return chunks[i]; }
		
		// No, is there any chunk that isn't in the closest area of players'?
		for (int i=0;i<cachesize;i++) if(!chunks[i].isUsed) { chunks[i] = new Chunk(x,y,true); chunks[i].isUsed = true; readChunkFile(x,y,chunks[i]); return chunks[i]; }
	...
	public Chunk findCachedChunk(int x, int y)
	{
		// Is the chunk cached?
	...
	public int findNewChunkID(int x, int y)
	{
		...
		// No, is there any empty chunk?
		for (int i=0;i<cachesize;i++) if(!chunks[i].isSet) { chunks[i] = new Chunk(x,y,true); return i; }
		
		// No, is there any chunk that isn't in the closest area of players'?
		for (int i=0;i<cachesize;i++) if(!chunks[i].isUsed) { chunks[i] = new Chunk(x,y,true); return i; }
	...
	public Chunk generateChunk(int cx, int cy, boolean used)
	{
		Chunk out = new Chunk(cx,cy,used);
	...
	// LOADING
	public void readChunkFile(int x, int y, Chunk target)
	{
		if(multiplayer)
		{
			...
			Chunk nout = new Chunk(x,y,true);
			target = nout;
		}
		else
		{
			try {
				FileInputStream in = new FileInputStream(saveDir + mapName + "/" + Chunk.getFilename(x,y));
	...
	{
		Chunk cin = findCachedChunk(x,y);
		if (cin != null)
		{
			saveChunkFile(x,y,cin);
		}
	}
	public void saveChunkFile(int x, int y, Chunk chunk)
	{
		...
		{
			checkDirs(x,y);
			fout = new FileOutputStream(saveDir + mapName + "/" + Chunk.getFilename(x,y));
	...
	public Block getBlock(int x, int y)
	{
		try
		{ 
			return new Block(x,y,grabChunk((x>>6),(y>>6)).getBlock(x&63,y&63));
		}
 	...
 	{
 		Block dc = getBlock(lolx,loly);
 		Block dt = getBlock(lolx+lolvx,loly+lolvy);
	...
	public int updateLook(Block block)
	{
