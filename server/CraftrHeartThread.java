...
public class CraftrHeartThread implements Runnable
{
	...
	public void run()
	{
		while(true)
		{
			...
			try
			{
				String tt = "http://admin.64pixels.org/heartbeat.php?name=" + se.name + "&port=" + se.po + "&players=" + se.countPlayers() + "&maxplayers=255&version=" + CraftrVersion.getProtocolVersion();
				tt = tt.replaceAll(" ","%20");
				URL u1 = new URL(tt);
				is = u1.openStream();
				int count = 1;
