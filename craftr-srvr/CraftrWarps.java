import java.util.*;
import java.io.*;
public class CraftrWarps
{
	ArrayList<CraftrWarp> warps;
	private DataInputStream in;
	private DataOutputStream out;
	public CraftrWarps()
	{
		warps=new ArrayList<CraftrWarp>(256);
	}
	public String readString()
	{
		...
		{
			System.out.println("Non-fatal CraftrWarps readString error!");
			return "";
		} 
	}
	...
	public void writeString(String s)
	{
		...
		{
			System.out.println("Non-fatal CraftrWarps writeString error!");
			try
			{
	...
	public void loadFile(String filename)
	{
		try
		{
			...
			warps=new ArrayList<CraftrWarp>(length+16);
			if(length>0)
			{
				for(int i=0;i<length;i++)
				{
					int x = in.readInt();
					int y = in.readInt();
					warps.add(new CraftrWarp(x,y,readString()));
				}
			}
			in.close();
			fin.close();
